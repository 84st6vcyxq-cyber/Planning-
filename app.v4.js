// ====== Planning 5x8 V4 ======
// Fix: calcul des jours en UTC (anti-DST / anti-décalage mois) => corrige “nuits -> repos”
// + HS auto +15 min/jour travaillé (M/A/N) + HS ajoutées
...
function calcTotalsForRange(dateStart, dateEndInclusive){
  ...
  cur.setHours(12,0,0,0); // midi : évite les soucis DST sur setDate()
...
function render(){
  ...
  for(let day=1; day<=last.getDate(); day++){
    ...
    code.textContent = shift; // petit debug (M/A/N/R/V)
...
function openDayDialog(key){
  ...
  const effShift = effectiveShiftForDate(new Date(...(() => { const p=parseISO(key); return [p.y, p.m, p.d]; })()));
